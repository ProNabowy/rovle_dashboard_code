...
export default function RenderTable({ columns, list, table, filterChildern }) {
  const {
    selectedEntries,
    setSelectedEntries,
    entries,
    ...
    <div className={`relative ${showPaginator ? "" : "pb-10"}`}>
      <div className="flex items-center justify-between flex-wrap p-3 sm:p-0">
        ...
        <div
          className={`${
            filterChildern
              ? "relative sm:me-8 mb-5 w-full sm:w-[400px]"
              : "relative sm:ms-8 mb-5 w-[90%] m-auto sm:w-[400px]"
          }`}
        >
          <Input
            type="text"
          ...
          <FontAwesomeIcon
            icon={faSearch}
            className="text-[#252525] text-[13px] absolute left-4 top-[50%] translate-y-[-50%]"
          />
        </div>
      </div>
      ...
      <div ref={table}>
        <DataTable
          ...
          rows={selectedEntries?.name}
          dataKey="id"
          emptyMessage={
            <h1 className="text-center my-5">No se encontraron datos</h1>
          }
      ...
      {list?.length ? (
        <div
          className={`flex items-center w-fit m-auto md:ms-3 lg:ms-0 md:absolute ${
            showPaginator ? "bottom-2 xl:bottom-0" : "-bottom-5"
          } left-8`}
        >
          <h2
            htmlFor="entries-dropdown"
            className="text-[#252525] font-medium me-3"
          >
            Mostrar
          </h2>
          ...
          {entries?.length < selectedEntries?.name ? (
            ...
            <div className="ps-2 !rounded-full">
              ...
              <label
                htmlFor="entries-dropdown"
                className="visually-hidden text-[16px] ms-3 text-[#252525] font-medium"
              >
                entradas
              </label>
            </div>
