...
const useDataGetter = (list) => {
    ...
    const { selectedEntries, setSelectedEntries, entries } = useTableEntries(list);
...
const data = (columns) => {
    ...
    return columns?.map(column => {
        ...
        return <Column field={column?.field} sortable header={column?.header} className={column?.classNames} body={column?.tamplate} />;
