...
export default function TableActions({
    ...
    const isHasPermissionToEdit = hasRoutePermissions(userPeressmisons, editKey);

    const isHasPermissionToDelete = hasRoutePermissions(userPeressmisons, deleteKey);
    ...
    const handleClick = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                handelDeleteFunction && handelDeleteFunction(rowData.id, list, state);
    ...
    return (
        ...
        <div className='flex items-center relative'>
            ...
            {reverseChildren ? children : null}

            {
                isHasPermissionToEdit
                    ?
                    <Link to={path} aria-label={`Navigate user to ${path}`} className={`cursor-pointer me-5 ${reverseChildren ? "ms-5" : "ms-10"} ${disableEdit ? "hidden" : ""}`}>
...

            {
                isHasPermissionToDelete
                    ?
                    <FontAwesomeIcon onClick={handleClick} className={`text-[20px] text-[#FF5C34] cursor-pointer ${disableDelete ? "hidden" : ""}`} icon={faTrashCan}></FontAwesomeIcon>
                    :
                    null
            }

            {reverseChildren ? null : children}
