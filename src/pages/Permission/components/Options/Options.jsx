...
export default function Options({ items, options, formik, onOptionChange }) {
                ...
                items?.map((item, index) => {
                    ...
                    return (
                        ...
                        <div key={index} className="flex items-center">
                            ...
                            <Checkbox disabled={item?.id === 517 || item?.id === 518 || item?.id === 520} inputId={item?.id} name={item?.id} value={item?.title} onChange={e => onOptionChange(e, item)} checked={options?.includes(item?.id)} />
