...
const useDataGetter = _ => {
    ...
    useEffect(() => {
        ...
        getUtility.getProducts().then(response => setProducts(response))
    ...
    const shopsBodyTemplate = (rowData) => {
        ...
        const itemBodyTamplate = (item, index, classNames) => <p key={index} className={`capitalize text-[13px] font-medium mb-1 rounded-[5px] text-[#58291E] ${classNames}`}>{item?.name}</p>;
    ...
    const packagesBodyTemplate = (rowData) => {
        ...
        const itemBodyTamplate = (item, index, classNames) => <p key={index} className={`capitalize text-[13px] font-medium mb-1 rounded-[5px] text-[#58291E] ${classNames}`}>{parseInt(item?.weight)} gm = {parseInt(item?.price)} Euro</p>;
    ...
    const columns = [
        ...
        { field: "commercial_name", header: "Nombre Productos", classNames: "!px-[0px]", tamplate: tableService.roasterNameBodyTemplate },
