...
export default function useHandleRoutes() {
    ...
    const { userPeressmisons, user } = useContext(AppContext);

    const isProvider = user?.roles?.[0]?.name === 'Provider';
    ...
    const routeConfig = [
        ...
        { path: "/origins/list/edit-origin", component: data.EditOrigin, permissionCheck: 'dashboard.origins.update' },
        //   Plans Routes
        ...
        { path: "/groups/clients", component: data.Customers, permissionCheck: 'dashboard.customers.list' },
        //   Orders Routes
        ...
        //   Profile Routes
        { path: "profile", component: data.Profile, permissionCheck: user?.id ? true : false },
